    ...
    <div className="space-y-6">
      ...
      {pedidos.length > 0 ? (
        pedidos.map((pedido) => (
          ...
          >
            <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleExpand(pedido.id)}>
              <div>
                <p className="font-semibold text-gray-800">Pedido #{pedido.id}</p>
                <p className="text-sm text-gray-600">Estado: {pedido.estado}</p>
                <p className="text-sm text-gray-600">Dirección: {pedido.direccion}</p>
                <p className="text-sm text-gray-600">Total: S/ {pedido.orden_total.toFixed(2)}</p>
              </div>
                ...
                {pedido.productos.map((producto, idx) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      Precio total: S/ {(producto.cantidad * producto.precio).toFixed(2)}
                    </p>
                  </div>
                ))}
              </div>
